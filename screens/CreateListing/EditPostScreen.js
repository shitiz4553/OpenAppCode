...
function EditPostScreen({navigation,route}){
    ...
    const [gender, setGender] = React.useState(item.genderPreference);
    const [propertyType, setPropertyType] = React.useState(item.propertyType);
    const [showDropDownType, setShowDropDownType] = React.useState("");
    const [propertyLocation, setPropertyLocation] = React.useState(item.propertyLocation);
    const [nearbyLocation, setNearbyLocation] = React.useState(item.nearby);
    const [price, setPrice] = React.useState(`${item.price}`);
    const [houseSize, setHouseSize] = React.useState(`${item.houseSize}`);
    const [builtYear, setBuiltYear] = React.useState(`${item.builtDate}`);
    const [rooms, setRooms] = React.useState(`${item.room}`);
    const [bathrooms, setBathrooms] = React.useState(`${item.bathroom}`);
    const [description, setDescription] = React.useState(item.description);
    const [propertyName, setPropertyName] = React.useState(item.propertyName);
    ...
    const handleSubmit = async () => {
        ...
        if (
          ...
          setLoading(true);
          const propertyRef = doc(FB_FIRESTORE, "properties",item.id);
        ...
        <View style={styles.body}>
          ...
          >
            <View style={styles.infoWrapper}>
              <Image source={{ uri: item.propertyImage }} style={styles.boxImage} />
